                    ...
                    <div className={styles.paramsFilm}>
                        ...
                        <div className={styles.paramsList}>
                            ...
                            {film.genres.map((genre, index) => {
                                 return <div className={styles.params} key={index}>{genre.name[0].toUpperCase() + genre.name.slice(1)}</div>
                            })}
                        </div>
                        <div className={styles.actorsList}>
                            ...
                            {film.persons.map((person, index) => {
                                if( index < 3) {
                                    return <div key={index} className={styles.actors}>
                                        <Image className={styles.actorImage} src={person.photo} alt={person.name} width={56} height={56}/>
            ...
            <div className={styles.wrapperFilms}>
                ...
                <div className={styles.wrapperItems}>
                    {film.similarMovies.map((f, index) => {
                        if(index < 7){
                            return <div key={index} className={styles.film}>
                                <Link className={styles.linkFilm} href={`/films/${f.id}`}>
                                    <Image className={styles.filmImage} src={f.poster.url} alt={f.name} width={153} height={235}/>
