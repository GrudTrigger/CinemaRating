...
export const Header = () => {
  ...
  useEffect(()=> {
     const getMovieByTitle = async () => {
       ...
       const query = queryBuilder.select(['id', 'name', 'rating', 'poster', 'year', 'genres.name', 'type']).filterExact('poster.url', SPECIAL_VALUE.NOT_NULL).filterExact('name', search).paginate(1, 4).build();
    ...
    <header>
      <div className={styles.container}>
        ...
        <div className={styles.wrapperSerch}>
          <input onChange={handleSearch} className={styles.input} type="text" placeholder="Поиск..." value={search} />
          <div className={styles.wrapperButtons}>
            <button className={styles.buttonHeader}>Войти</button>
          </div>
        </div>
        {searchFilm &&  <SearchFilm searchFilm={searchFilm}/>}
      </div>
    </header>
  );
};
